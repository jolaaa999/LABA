...
const agentCase: ProjectCaseStudy = {
  overview:
    'Campus Agent Workshop 旨在探索如何把社团活动里的重复协调工作，收敛成可拆解、可调用工具、可回写结果的 Agent 工作流设计。',
  problem:
  ...
  architecture: {
    ...
    nodes: [
      { id: 'intent', label: 'Intent', detail: '活动目标与约束' },
      { id: 'planner', label: 'Planner', detail: '步骤拆解与优先级' },
      { id: 'tools', label: 'Tools', detail: 'MCP / 本地脚本' },
      { id: 'memory', label: 'Memory', detail: '短时上下文与清单' },
      { id: 'writeback', label: 'Write-back', detail: '结果归档' },
    ],
  },
  workflow: [
    {
    ...
    {
      title: 'Observe & write back',
      detail: '目标：记录结果，回写到共享笔记。',
    },
  ],
  results: [
    {
      label: 'Illustrative demo path',
      detail: '目标演示路径为 Task → Plan → Tool → Result；尚未作为生产系统验证。',
    },
    {
      label: 'Shared vocabulary (intended)',
  ...
  artifacts: [
    { label: 'Design notes', kind: 'notes', note: 'Illustrative only' },
    { label: 'Repository', kind: 'repo', note: 'Not published' },
  ],
  collaborators: [{ name: 'Dev Placeholder', role: 'Engineering' }],
  nextSteps: [
    '若进入实现：补齐评测用例与失败回放。',
    '收敛工具白名单与权限模型。',
  ],
...
const researchCase: ProjectCaseStudy = {
  overview:
    'Attention Ablation Lab 规划用对照实验整理 Transformer attention 的复现与消融记录，强调可复查的配置与失败样本归档方式。',
  question:
  ...
  experiment: {
    ...
    metrics: ['Validation loss', 'Attention entropy proxy', 'Failure rate'],
    controls: ['Baseline attention', 'Ablated heads / patterns'],
  },
  observations: [
  ...
  results: [
    {
      label: 'Configuration ledger',
      detail: '预期产物：可对照的实验配置记录结构。',
    },
    {
      label: 'Failure-case index',
  ...
  artifacts: [
    { label: 'Experiment notes', kind: 'notes', note: 'Illustrative outline' },
    { label: 'Config templates', kind: 'other', note: 'Design only' },
  ],
  collaborators: [{ name: 'Dev Placeholder', role: 'Research' }],
  nextSteps: [
    '有真实跑数后再补对照维度与可视化。',
    '把失败样本分类写成短文（若实验执行）。',
  ],
...
const toolCase: ProjectCaseStudy = {
  overview:
    'Paper Pipeline CLI 计划把论文阅读、复现清单与实验记录串成命令行流水线，减少文档在协作中的漂移。',
  problem:
  ...
  architecture: {
    ...
    nodes: [
      { id: 'parse', label: 'Parse', detail: '论文笔记结构化' },
      { id: 'checklist', label: 'Checklist', detail: '复现清单' },
      { id: 'runlog', label: 'Run log', detail: '实验记录' },
      { id: 'export', label: 'Export', detail: '汇总输出' },
    ],
  },
  workflow: [
    { title: 'Ingest notes', detail: '目标：从 Markdown 读取基础字段。' },
    { title: 'Build checklist', detail: '目标：生成可勾选复现项。' },
    { title: 'Append run log', detail: '目标：写入一次实验摘要。' },
    { title: 'Export summary', detail: '目标：输出可读汇总。' },
  ],
  usage: [
    {
  ...
  results: [
    {
      label: 'CLI shape (intended)',
      detail: '设计目标：命令与目录约定可被其他工具型项目参考。',
    },
    {
      label: 'Intended status: Shared',
  ...
  artifacts: [
    { label: 'CLI sketch', kind: 'repo', note: 'Not published' },
    { label: 'Usage notes', kind: 'notes', note: 'Illustrative' },
  ],
  collaborators: [{ name: 'Dev Placeholder', role: 'Tooling' }],
  nextSteps: [
...
export const projects: Project[] = [
  ...
  },
  {
    ...
    caseStudy: {
      ...
      workflow: [
        { title: 'Build eval set', detail: '目标：标注一组可复查问答。' },
        { title: 'Sweep strategies', detail: '目标：对照 chunk 与检索参数。' },
        { title: 'Archive misses', detail: '目标：失败样本分类入库。' },
      ],
      results: [
        {
          label: 'Reusable notes (target)',
          detail: '设计目标：形成可引用的策略备忘结构；尚未验证采用效果。',
        },
      ],
      limitations: ['未接生产检索服务；无真实评测跑数。'],
      artifacts: [{ label: 'Lab notes', kind: 'notes', note: 'Illustrative' }],
      nextSteps: ['若实现：补自动评测脚本。'],
    },
  },
  {
    slug: 'vision-baseline-kit',
    title: 'Vision Baseline Kit',
    category: 'research',
    summary:
    ...
    caseStudy: {
      ...
      results: [
        {
          label: 'Starter kit (planned)',
          detail: '预期产物：可用于 workshop 演示的模板结构。',
        },
      ],
      limitations: ['覆盖任务面窄；尚无真实训练产物。'],
      artifacts: [{ label: 'Kit notes', kind: 'notes', note: 'Illustrative' }],
  ...
  },
  {
    ...
    caseStudy: {
      ...
      usage: [
        { title: 'Install locally', detail: '拟定：在开发机挂载样例 server。' },
        { title: 'Call tools', detail: '拟定：从 Agent 侧调用白名单工具。' },
      ],
      architecture: {
      ...
      artifacts: [{ label: 'Toolbench', kind: 'repo', note: 'Not published' }],
      nextSteps: ['补安全沙箱说明。'],
    },
  },
  {
    slug: 'club-ops-assistant',
    title: 'Club Ops Assistant',
    category: 'ai-engineering',
    summary:
    ...
    caseStudy: {
      ...
      workflow: [
        { title: 'Collect inputs', detail: '目标：收集活动要点与时间。' },
        { title: 'Draft', detail: '目标：生成通知 / 清单草稿。' },
        { title: 'Review', detail: '目标：人工修订后发出。' },
      ],
      results: [
        {
          label: 'Draft prototype (intended)',
  ...
  },
  {
    ...
    caseStudy: {
      ...
      workflow: [
        { title: 'Bootstrap', detail: '目标：复制目录约定。' },
        { title: 'Fill checklist', detail: '目标：对照提交项。' },
        { title: 'Demo script', detail: '目标：准备演示路径。' },
      ],
      results: [
        {
          label: 'Pack outline (illustrative)',
          detail: '当前为占位材料结构，待真实赛事资料替换。',
        },
      ],
      limitations: ['非特定赛事官方材料。'],
      artifacts: [{ label: 'Starter docs', kind: 'notes', note: 'Illustrative' }],
      nextSteps: ['按真实赛事更新清单。'],
    },
  },
]
